...
export function FieldStaffAttendanceReport() {
  ...
  const roles = ['all', 'field_worker', 'field_supervisor'];
  ...
  async function fetchAttendanceData() {
    try {
      ...
      const { data: employeesData, error: empError } = await supabase
        ...
        .in('role', ['field_worker', 'field_supervisor'])
        .order('full_name');
                ...
                {dailyAttendance.map((day) => {
                  ...
                  const getStatusColors = () => {
                    switch (day.status) {
                      ...
                      case 'wfh': return 'bg-blue-50 border-blue-300 text-blue-600';
                  ...
                  return (
                    <div
                      key={day.date}
                      className={`border rounded-lg p-1 sm:p-2 flex flex-col items-center justify-center ${getStatusColors()} min-h-[60px] sm:min-h-[80px]`}
                    >
                      ...
                      {day.note && (
                        <div className="text-[9px] sm:text-[10px] text-gray-600 text-center mt-1 max-w-full truncate px-1">
                          {day.note}
                        </div>
                      )}
                    </div>
